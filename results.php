...
if(isset($_POST['esubmit'])){
        ...
        if($conn){
          ...
          if(mysqli_num_rows($result)>0){
            ...
            echo "<script>
              alert('You are authenticated  " .$_SESSION['egid'] .   "');
            
            </script>";
            $query = "SELECT r.pid FROM  result r  WHERE r.decision='accept'
        AND r.pid=$epid HAVING count(r.pid)>=3";
                $result = mysqli_query($conn, $query);  
                if(mysqli_num_rows($result)>0){
           
            
            echo "<script>
              alert('your poster has been accepted ! congrats !');
            ...
            echo "<script>
              alert('your poster is yet to be reviewed!');
              window.location.href='Poster.php';
                            </script>";
            ...
            $query = "SELECT r.pid FROM  result r  WHERE r.decision='reject'
        AND r.pid=$epid  HAVING count(r.pid)>=3";
                $result = mysqli_query($conn, $query);  
                if(mysqli_num_rows($result)>0){
           
            echo "<script>
              alert('your poster has been rejected .Better luck next time!');
              window.location.href='Poster.php';
                            </script>"; }
